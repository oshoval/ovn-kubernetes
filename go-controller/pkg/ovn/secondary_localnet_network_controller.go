...
func (oc *SecondaryLocalnetNetworkController) checkBridgeMapping(obj interface{}) error {
	...
	err := fmt.Errorf("failed to find bridge mapping for network: %q, node %q, ovn-bridge-mappings: %s", networkName, pod.Spec.NodeName, ovnBridgeMappings)
	oc.recordPodErrorEvent(pod, err)
	return err
}

func (oc *SecondaryLocalnetNetworkController) getNetworkName() string {
