...
func (oc *SecondaryLocalnetNetworkController) checkBridgeMapping(obj interface{}) error {
	...
	if oc.nodeName != pod.Spec.NodeName {
		return nil
	}

	knownNad, err := oc.podAttachedToKnownNAD(pod)
	if err != nil {
		return err
	}
	if !knownNad {
...
func (oc *SecondaryLocalnetNetworkController) podAttachedToKnownNAD(pod *corev1.Pod) (bool, error) {
...
func (oc *SecondaryLocalnetNetworkController) networkMappedToBridge(pod *corev1.Pod) error {
	ovnBridgeMappings := oc.openvSwitch.ExternalIDs["ovn-bridge-mappings"]
	...
	err := fmt.Errorf("failed to find bridge mapping for network: %q, node %q, ovn-bridge-mappings: %q", networkName, oc.nodeName, ovnBridgeMappings)
